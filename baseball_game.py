...
def get_strikes_or_ball(user_input_number, random_number):
    # '''
    # Input:
    #   - user_input_number : 문자열값으로 사용자가 입력하는 세자리 정수 + 올바르게 입력된 정수
    ...
    if num_strike == 3:
        return [num_strike, num_ball]

    # ball 계산
    for i in range(3):
        # 사용자가 입력한 숫자의 특정 수와 일치하는 랜덤 넘버의 인덱스 반환 -> 불일치 시 -1 반환
        idx = random_number.find(user_input_number[i])
        if idx != -1 and idx != i:
            num_ball += 1
